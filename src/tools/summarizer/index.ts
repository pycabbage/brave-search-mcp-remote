...
import { type SummarizerQueryParams, summarizerQueryParams } from "./params"
import type { SummarizerSearchApiResponse } from "./types"
...
export const execute = async (
  ...
  try {
    const { summary } = await pollForSummary(params, apiKey)
...
const pollForSummary = async (
  ...
  while (!result && attempts > 0) {
    try {
      const response = await API.issueRequest("summarizer", params, apiKey)
