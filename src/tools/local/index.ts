...
import { formatWebResults } from "../web/index"
import webParams, { type QueryParams as WebQueryParams } from "../web/params"
import type { WebSearchApiResponse } from "../web/types"
import type {
  DayOpeningHours,
  LocationDescription,
  LocationResult,
...
export const execute = async (params: WebQueryParams, apiKey: string) => {
  // Make sure both 'web' and 'locations' are in the result_filter
  params = {
    ...params,
    result_filter: [...(params.result_filter || []), "web", "locations"],
  }

  // Starts with a web search to retrieve potential location IDs
  const { locations, web: web_fallback } = await API.issueRequest<"web">(
    "web",
    params,
    apiKey
  )
  ...
  const locationIDs = (locations?.results || [])
    .map((poi) => poi.id as string)
    .slice(0, 20)
  ...
  return {
    content: formatLocalResults(locations.results, descriptions.results).map(
      (formattedPOI) => ({
        type: "text" as const,
        text: formattedPOI,
      })
...
const buildFallbackWebResponse = (
  web_fallback: WebSearchApiResponse["web"]
): CallToolResult => {
  if (!web_fallback || web_fallback.results.length === 0)
