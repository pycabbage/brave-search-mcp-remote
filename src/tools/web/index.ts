...
export const execute = async (params: QueryParams, apiKey: string) => {
  ...
  const { web, faq, discussions, news, videos, summarizer } =
    await API.issueRequest("web", params, apiKey)
...
const formatDiscussionsResults = (
...
const formatVideoResults = (videos: Videos): FormattedVideoResults => {
  return (videos.results || []).map(
    ({ url, age, title, description, video, thumbnail }) => ({
